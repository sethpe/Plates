            ...
            HStack(spacing: 0) {
                VStack(alignment: .leading, spacing: 7) {
                    ...
                    .sheet(isPresented: $shouldPresentEditWorkout, content: {
                        AddWorkout(shouldPresentAddNewWorkout: self.$shouldPresentEditWorkout, name: workout.wName, notes: workout.wNotes, categoryIndex: WorkoutCategory.allCases.firstIndex(of: workout.wCategory) ?? 0, workoutToEdit: workout).environment(\.managedObjectContext, self.managedObjectContext)/*.environmentObject(self.appSettings)*/
                    })
...
struct WorkoutRow_Previews: PreviewProvider {
    ...
    static var previews: some View {
        ...
        pWorkout.isFavorite = true
        //        pWorkout.lastTrainedAt = Date().addingTimeInterval(-1000)
        pWorkout.category = WorkoutCategory.fullBody.rawValue
